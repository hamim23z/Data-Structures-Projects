...
bool test_items(sequence &test, size_t s, size_t i, double items[]) {
  ...
  cout << "The cursor should be at item [" << i << "]"
       << " of the sequence\n";
  ...
  if ((i != s) &&
      !test.is_item()) { // The test.is_item( ) function returns false too soon.
    ...
    answer = false;
  } else if (test.is_item()) { // The test.is_item( ) function returns true
...
int test5() {
  ...
  // Test copying of an empty sequence. After the copying, we change the
  // original.
  ...
  if (!correct(copy2, 2 * original.DEFAULT_CAPACITY,
               2 * original.DEFAULT_CAPACITY - 1, items))
    return 0;

  // Test copying of a sequence with cursor near the middle.
  cout << "Copy constructor test: for a sequence with cursor near middle."
       << endl;
  original.insert(2);
  for (i = 1; i < original.DEFAULT_CAPACITY; i++)
    original.advance();
  // Cursor is now at location [DEFAULT_CAPACITY] (counting [0] as the first
  // spot).
  sequence copy3(original);
  original.start();
  original.advance();
  original.remove_current(); // Removes 2 from the original, but not the copy.
  if (!correct(copy3, 2 * original.DEFAULT_CAPACITY, original.DEFAULT_CAPACITY,
               items))
    return 0;
  ...
  original.advance();
  original.remove_current(); // Removes 2 from the original, but not the copy.
  if (!correct(copy5, 2 * original.DEFAULT_CAPACITY,
...
int test6() {
  ...
  // Test copying of a sequence with cursor near the middle.
  cout << "Assignment operator test: for a sequence with cursor near middle."
       << endl;
  original.insert(2);
  for (i = 1; i < original.DEFAULT_CAPACITY; i++)
    original.advance();
  ...
  // Test copying of a sequence with no current item.
  cout << "Assignment operator test: for a sequence with no current item."
       << endl;
  original.insert(2);
  while (original.is_item())
    original.advance();
...
int test7() {
  ...
  for (i = 1; i <= 2 * testa.DEFAULT_CAPACITY; i++)
    testa.attach(i);
  if (!correct(testa, 2 * testa.DEFAULT_CAPACITY,
               2 * testa.DEFAULT_CAPACITY - 1, items))
    return 0;
  ...
  for (i = 2 * testi.DEFAULT_CAPACITY; i >= 1; i--)
    testi.insert(i);
  if (!correct(testi, 2 * testi.DEFAULT_CAPACITY, 0, items))
    return 0;
